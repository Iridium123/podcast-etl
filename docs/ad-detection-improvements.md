...
4. ~~**Make the LLM prompt configurable** — The classification prompt is hardcoded in `transcription.py`. Allow prompt variants to be swapped without code changes, supporting A/B experimentation.~~ **Done in PR-B (thin-scorer).** `load_prompt(name)` in `podcast_etl.detectors.transcription` resolves `prompts/<name>.txt`; the prompt is selected via `ad_detection.llm.prompt` in `feeds.yaml` and the eval matrix config. An unknown prompt name fails config validation early. Both production (`AnthropicProvider.classify`) and eval (`eval/label.py`) go through the same `load_prompt` + `classify()` path.
...
9. ~~**Reuse existing production transcripts in eval** — The `run_eval` runner currently always re-transcribes audio for each whisper config, even when an existing transcript is on disk in the production output directory under `<podcast_dir>/transcripts/`.~~ **Done in PR-B (thin-scorer).** `eval/label.py`'s `_reuse_production_transcript` checks `episode.status['detect_ads'].result['whisper']`; if it matches `normalize_whisper_config(eval_whisper)`, the on-disk transcript is loaded instead of re-transcribing. A single in-memory cache is shared across all eval configs in a run so configs with identical whisper settings transcribe each episode only once.
