...
4. `cli.py` — Click commands (`add`, `fetch`, `run`, `reset`, `delete`, `status`, `poll`); registers built-in steps at import time via `register_step()`; `find_feed_config(config, identifier)` resolves a feed by name or URL; `find_podcast_dir(output_dir, url)` finds the podcast directory matching a feed URL; `reset_feed_data(output_dir, url)` finds and deletes it; `delete_feed(config, config_path, identifier)` removes a feed from config and deletes its data
