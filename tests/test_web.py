...
def test_trusted_origins_accepts_whitelisted_origin(tmp_path: Path) -> None:
    """An Origin listed in web.trusted_origins must be accepted even when Host differs.

    This is the Cloudflare Tunnel / reverse-proxy case: the browser sends
    Origin: https://podcast-etl.example.com but the Host header reaching
    the app might be rewritten (e.g. to the backend container name).
    """
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {"trusted_origins": ["https://podcast-etl.example.com"]},
    })
    app = create_app(cfg_path, start_poller=False)
    client = TestClient(app)
    response = client.post(
        "/poll/pause",
        headers={
            "origin": "https://podcast-etl.example.com",
            "host": "podcast-etl:8000",
        },
    )
    assert response.status_code == 200


def test_trusted_origins_rejects_non_whitelisted_even_when_list_set(tmp_path: Path) -> None:
    """Setting trusted_origins must not disable the check for other origins."""
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {"trusted_origins": ["https://podcast-etl.example.com"]},
    ...
    response = client.post(
        "/poll/pause",
        headers={"origin": "https://evil.example.com", "host": "podcast-etl:8000"},
    )
    assert response.status_code == 400


def test_trusted_origins_multiple_entries(tmp_path: Path) -> None:
    """Multi-host deployment: LAN + Tailscale + Cloudflare all whitelisted."""
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {
    ...
    for origin in (
        ...
        response = client.post(
            "/poll/pause",
            headers={"origin": origin, "host": "podcast-etl:8000"},
        )
        assert response.status_code == 200, f"Origin {origin!r} should be accepted"


def test_trusted_origins_case_insensitive(tmp_path: Path) -> None:
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {"trusted_origins": ["https://Podcast-ETL.Example.com"]},
...
def test_trusted_origins_trailing_slash_tolerated(tmp_path: Path) -> None:
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {"trusted_origins": ["https://podcast-etl.example.com/"]},
...
def test_trusted_origins_scheme_mismatch_rejected(tmp_path: Path) -> None:
    """http://X and https://X are different origins; match must be exact."""
    cfg_path = _write_config(tmp_path, {
        "feeds": [],
        "defaults": {"output_dir": str(tmp_path / "output"), "pipeline": ["download"]},
        "web": {"trusted_origins": ["https://podcast-etl.example.com"]},
    ...
    response = client.post(
        "/poll/pause",
        headers={
            "origin": "http://podcast-etl.example.com",
            "host": "podcast-etl:8000",
        },
    )
    assert response.status_code == 400
