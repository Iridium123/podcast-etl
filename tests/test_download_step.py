...
from podcast_etl.models import Episode, Podcast, episode_basename
...
def _mock_retry_client(chunks: list[bytes]):
    """Return a mock retry_client() that yields a client with streaming response."""
    mock_response = MagicMock()
    mock_response.raise_for_status = MagicMock()
    mock_response.iter_bytes = MagicMock(return_value=iter(chunks))

    mock_client = MagicMock()
    mock_client.stream = MagicMock(return_value=contextmanager(lambda *a, **kw: (yield mock_response))())
...
def test_process_extracts_mp3_extension_from_url(tmp_path: Path):
    ...
    with patch("podcast_etl.steps.download.retry_client", _mock_retry_client([b"audio data"])):
...
def test_process_skips_download_if_file_already_exists(tmp_path: Path):
    ...
    with patch("podcast_etl.steps.download.retry_client") as mock_client:
        result = DownloadStep().process(ep, ctx)
...
def test_process_downloads_and_saves_file(tmp_path: Path):
    ...
    with patch("podcast_etl.steps.download.retry_client", _mock_retry_client([audio_content])):
        result = DownloadStep().process(ep, ctx)
...
def test_process_creates_audio_directory(tmp_path: Path):
    ...
    with patch("podcast_etl.steps.download.retry_client", _mock_retry_client([b"data"])):
...
def test_process_propagates_http_error(tmp_path: Path):
    ...
    with patch("podcast_etl.steps.download.retry_client", return_value=mock_client):
